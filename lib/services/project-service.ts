...
import { db } from "@/db";
import { activityLogs, assets, projects } from "@/db/schema";
import type { NewProjectRecord, ProjectChanges, ProjectFilters } from "@/lib/models/project";
import { notifyProjectCreated, notifyProjectUpdated } from "@/lib/services/notification-service";
...
export async function createProjectWithActivity(values: NewProjectRecord, actor: ActivityActor) {
  ...
  await notifyProjectCreated(project, actor.userId, actor.name);
  return project;
}
